...
//   DB                  D1 binding (consent ledger + rate limit)
//
// Single opt-in: Resend Audiences has no native double opt-in, so the consent
// checkbox on the form is the record of consent (filed in the D1 ledger).
// ─────────────────────────────────────────────────────────────────────────────
import { corsFor, replyWith, clientIp, EMAIL_RE } from '../lib/http.js'
import { rateLimit, persistConsent } from '../lib/db.js'
...
export async function handler(event, env = {}) {
  ...
  const limiter = await rateLimit(env, clientIp(event), { storeName: 'newsletter-rate' })
  if (!limiter.ok) {
  ...
  try {
    ...
    if (res.ok) { await persistConsent(env, consent); await limiter.commit(); return reply(200, { ok: true }) }
    ...
    if (res.status === 409 || msg.includes('already')) { await persistConsent(env, consent); await limiter.commit(); return reply(200, { ok: true, already: true }) }
