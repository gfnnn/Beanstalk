...
| `tests/enquire.test.js` · `tests/filter.test.js` · `tests/loadmore.test.js` | the synchronous, correctness-critical logic of the matching `src/js/modules/` modules, driven under **jsdom** (step-gating validation, conditional-field disabling, filter/sort/window cooperation). `loadmore` also covers the shared button busy-state (`.btn-spinner` / `aria-busy` swapped in on click, restored once the revealed batch settles). |
| `tests/loader.test.js` | the full-page preloader, both halves — `src/build/loader.js` (`injectPageLoader`: critical `<style>` into the head + overlay after `<body>`, idempotent, accessible label, CSS failsafe) and `src/js/modules/loader.js` (`initPageLoader` under **jsdom**: dismiss on ready, the instant-fade vs. real-load `.pl-finishing` quick-cycle branch, reduced-motion immediate removal). |
