...
const LANES = {
  portfolio: { widths: [400, 800, 1200], aspect: 3 / 4, position: 'attention' },
...
async function processOne({ src, name, lane, outDir, crop, sharpen }) {
  ...
  for (const width of recipe.widths) {
    // Base pipeline: apply EXIF rotation, then either cover-crop to the lane
    // aspect or just downscale by width keeping the source aspect (--no-crop).
    let height
    const make = () => {
